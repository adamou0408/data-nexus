-- ============================================================
-- V021: Create remaining business tables for POC end-to-end testing
-- These tables were registered as authz_resource but had no physical PG table.
-- Now creating real tables with realistic Phison sample data.
-- ============================================================

-- ─── wip_inventory: 在製品庫存追蹤 (MRP > Lot Tracking) ───
CREATE TABLE IF NOT EXISTS wip_inventory (
    wip_id        TEXT PRIMARY KEY,
    lot_id        TEXT NOT NULL,
    product_line  TEXT NOT NULL,
    chip_model    TEXT NOT NULL,
    stage         TEXT NOT NULL,            -- wafer_prep, die_attach, wire_bond, molding, cp_test, ft_test, packing
    quantity      INTEGER NOT NULL,
    yield_rate    NUMERIC(5,2),             -- percentage
    operator      TEXT,
...
CREATE INDEX idx_wip_inventory_product_line ON wip_inventory(product_line);
CREATE INDEX idx_wip_inventory_lot_id ON wip_inventory(lot_id);
...
-- ─── cp_ft_result: CP/FT 測試結果 (MRP > Yield Analysis) ───
...
CREATE INDEX idx_cp_ft_result_product_line ON cp_ft_result(product_line);
CREATE INDEX idx_cp_ft_result_lot_id ON cp_ft_result(lot_id);
...
-- ─── npi_gate_checklist: NPI 階段閘門審查 (MRP > NPI) ───
CREATE TABLE IF NOT EXISTS npi_gate_checklist (
    gate_id       SERIAL PRIMARY KEY,
    product_line  TEXT NOT NULL,
    chip_model    TEXT NOT NULL,
    gate_phase    TEXT NOT NULL,             -- G0_concept, G1_feasibility, G2_dev, G3_qualification, G4_mass_production
...
CREATE INDEX idx_npi_gate_product_line ON npi_gate_checklist(product_line);
...
-- ─── reliability_report: 可靠性測試報告 (Quality > Reliability) ───
CREATE TABLE IF NOT EXISTS reliability_report (
    report_id      TEXT PRIMARY KEY,
    product_line   TEXT NOT NULL,
    chip_model     TEXT NOT NULL,
    test_type      TEXT NOT NULL,           -- HTOL, TC, UHAST, ESD, Latch-up
    test_condition TEXT NOT NULL,           -- e.g., "125°C / 1000hrs"
    sample_size    INTEGER NOT NULL,
    pass_count     INTEGER NOT NULL,
    fail_count     INTEGER NOT NULL,
    duration_hours INTEGER,
    failure_mode   TEXT,                    -- NULL if all passed
    engineer       TEXT NOT NULL,
    status         TEXT NOT NULL DEFAULT 'in_progress', -- in_progress, passed, failed
    created_at     TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE INDEX idx_reliability_product_line ON reliability_report(product_line);
...
INSERT INTO reliability_report (report_id, product_line, chip_model, test_type, test_condition, sample_size, pass_count, fail_count, duration_hours, failure_mode, engineer, status) VALUES
    ('REL-001', 'SSD',  'E18',   'HTOL',    '125°C / 1000hrs',    77, 77,  0, 1000, NULL,                     'user:huang_qa', 'passed'),
    ('REL-002', 'SSD',  'E18',   'TC',      '-40~125°C / 500cyc', 77, 77,  0,  NULL, NULL,                    'user:huang_qa', 'passed'),
    ('REL-003', 'SSD',  'E18',   'UHAST',   '130°C/85%RH/96hrs',  77, 76,  1,   96, 'Corrosion on pad',       'user:huang_qa', 'failed'),
    ('REL-004', 'SSD',  'E26',   'HTOL',    '125°C / 1000hrs',    77, 77,  0, 1000, NULL,                     'user:huang_qa', 'passed'),
    ('REL-005', 'SSD',  'E26',   'ESD',     'HBM 2kV / CDM 500V', 30, 30,  0,  NULL, NULL,                   'user:huang_qa', 'passed'),
    ('REL-006', 'eMMC', 'PS8211','HTOL',    '125°C / 1000hrs',    77, 77,  0, 1000, NULL,                     'user:huang_qa', 'passed'),
    ('REL-007', 'eMMC', 'PS8211','TC',      '-40~125°C / 500cyc', 77, 77,  0,  NULL, NULL,                    'user:huang_qa', 'passed'),
    ('REL-008', 'eMMC', 'PS5021','HTOL',    '125°C / 500hrs',     77, 77,  0,  500, NULL,                     'user:huang_qa', 'in_progress'),
...
CREATE TABLE IF NOT EXISTS rma_record (
    ...
    root_cause         TEXT,                  -- NULL until analysis complete
    status             TEXT NOT NULL DEFAULT 'open', -- open, analyzing, resolved, closed
...
CREATE INDEX idx_rma_product_line ON rma_record(product_line);
CREATE INDEX idx_rma_region ON rma_record(region);
...
INSERT INTO rma_record (rma_id, customer, product_line, chip_model, quantity, failure_description, root_cause, status, region, created_by, assigned_to) VALUES
    ...
    ('RMA-005', 'Transcend',  'SD',   'PS2251',  800, 'CRC error in cold temperature (-20°C)',   'Clock timing margin too tight',  'resolved', 'TW', 'user:lee_sales',   'user:wang_pe'),
...
-- ─── price_book: 產品定價表 (Sales > Pricing) ───
CREATE TABLE IF NOT EXISTS price_book (
    price_id       SERIAL PRIMARY KEY,
    product_line   TEXT NOT NULL,
    chip_model     TEXT NOT NULL,
    customer_tier  TEXT NOT NULL,            -- tier1, tier2, tier3, distributor
    unit_price     NUMERIC(10,2) NOT NULL,
    cost           NUMERIC(10,2) NOT NULL,
    margin         NUMERIC(5,2) NOT NULL,   -- percentage (restricted for non-finance)
...
CREATE INDEX idx_price_book_product_line ON price_book(product_line);
CREATE INDEX idx_price_book_customer_tier ON price_book(customer_tier);
