        ...
        LOOP
            ...
            IF v_profile.denied_columns IS NOT NULL THEN
                ...
                LOOP
                    ...
                    NULL; -- placeholder: column masking views are generated by authz_sync_column_views()
                END LOOP;
            END IF;
...
AuthZ Admin lives at `apps/authz-admin/` as an independent app, alongside `apps/workbench/` (Config-SM business modules) and `apps/portal/` (traditional web). Shared AuthZ client SDK lives at `packages/authz-client/` (published as `@nexus/authz-client`). The AuthZ core service lives at `services/authz-service/`.
...
- `services/` — Backend services (authz-service, identity-sync, sync-scheduler)
...
- Health probes: authz-service needs liveness (process alive), readiness (DB connected + Casbin loaded + sync not stale), startup (initial policy load)
- HPA: min 3 replicas for authz-service (critical path); scale on CPU + p99 latency
- PDB: minAvailable=2 for authz-service, minAvailable=1 for PostgreSQL
- Network Policies: only authz-service/pgbouncer/migration can reach PostgreSQL; only frontend apps + agent + sync can reach authz-service
- Casbin pattern: centralized (Casbin in authz-service pods) initially; sidecar pattern (Casbin per-app pod via OPAL) if latency becomes bottleneck
...
- Cache invalidation: PG trigger fires NOTIFY on policy/role_permission/subject_role changes → authz-service listener invalidates L1 Redis → L2 refreshes on next miss
...
# VIII. Implementation Roadmap v2.3

| Phase | Deliverable | Paths | Dependency |
|-------|-------------|-------|------------|
| 1 | AuthZ schema DDL deployed (all tables) | ALL | PostgreSQL 14+ |
...
| **18** | **K8s: External Secrets, NetworkPolicy, PDB, HPA for authz-service** | ALL | Phase 17 |
...
│
├── services/
│   ├── authz-service/                   ← AuthZ core service (Layer 1.5)
│   │   ├── sql/
...
```
# ============================================================
# services/authz-service/api/admin-crud.js
...
```typescript
// services/authz-service/sync/db-adapter.ts
...
│   ├── authz-service/                   ← AuthZ core API (Node.js / Go)
│   │   ├── Dockerfile
│   │   ├── package.json
│   │   ├── src/
...
packages/authz-client ────────────────┐   packages/db-adapters
       │         │         │          │          │
       ▼         ▼         ▼          ▼          ▼
apps/portal  apps/workbench  apps/authz-admin  services/authz-service
                                                      │
                                                      ▼
...
- `database/` is consumed by `services/authz-service` and `deploy/helm/charts/db-migration/`
...
│  │ api.phison.internal/authz/*    → authz-service:8080   │       │
...
The **readiness probe** is especially critical: if authz-service is not ready (DB connection lost, Casbin cache stale), all dependent services (portal, workbench, agent) should stop routing to that instance. This prevents authorization decisions being made against stale or empty policy caches.
...
```yaml
# authz-service HPA — scale on request latency, not just CPU
...
```yaml
# authz-service must always have 2+ pods running during rolling updates
apiVersion: policy/v1
kind: PodDisruptionBudget
metadata:
  name: authz-service-pdb
spec:
  minAvailable: 2
  selector:
    matchLabels:
...
```yaml
# Only authz-service can talk to PostgreSQL
...
---
# Only apps can talk to authz-service
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
  name: authz-service-access
spec:
  podSelector:
    matchLabels:
      app: authz-service
...
```
portal ──HTTP──→ authz-service (3 replicas, each runs Casbin in-process)
                      │
                      ▼
                  PostgreSQL (policy store)
```
- Casbin runs inside authz-service pods
...
└──────────────────────┼───────┘
                       │ policy sync
                       ▼
                authz-service (policy distribution only)
```
...
This is critical for authz-service: if a pod is killed mid-request, an in-flight `authz_check()` could fail, causing a 403 to the end user. The preStop hook + terminationGracePeriodSeconds ensure in-flight requests complete.

### 14.2.10 Observability

```yaml
# ServiceMonitor for Prometheus (authz-service exposes /metrics)
apiVersion: monitoring.coreos.com/v1
kind: ServiceMonitor
metadata:
  name: authz-service-monitor
spec:
  selector:
    matchLabels:
      app: authz-service
...
Key metrics to expose from authz-service:
...
authzService:
  replicaCount: 3
  image:
    repository: phison/authz-service
    tag: latest
  resources:
    requests:
      cpu: 250m
...
│  │  → authz-service receives notification                    │    │
...
```sql
-- Trigger on policy change → notify authz-service
...
2. Cache-first degraded mode:
   → If Policy Store is unreachable, authz-service continues serving from L1 Redis cache
...
**Scenario**: As more modules, resources, and fine-grained policies are added, Casbin's in-memory policy set grows. Each authz-service pod holds the full policy set. With 10,000 rules × 3 pods = 30,000 rule copies.
...
```
1. authz-service exposes REST API (language-agnostic)
