...
**新近完成（本 session 落地）:**
- [x] **M4-KEYCLOAK-SSO-S1 (additive scaffolding)** — 2026-04-29。M4 hard gate item #3 開工,只做 additive(零 destructive、零 DB row 變動)以避開 mock-user 移除的 constitutional consent。落地:(1) Keycloak realm `data-nexus` 建立於 shared `192.168.40.60:8080`(獨立 realm,不影響 edgepolicy/edgepolicy-test);(2) 兩個 OIDC client — `authz-dashboard`(public, PKCE S256, redirect `localhost:13173/*`,含 `audience-authz-api` + `groups` protocol mapper)+ `authz-api`(bearer-only,token-validation 目標);(3) 5 個 realm role 對齊 DB authz_role(`SYSADMIN`/`ADMIN`/`AUTHZ_ADMIN`/`BI_USER`/`ETL_SVC`);(4) smoke user `adam_ou`(SYSADMIN,**無 credential** — 密碼由 Adam 在 UI 自設,不過 transcript);(5) `.env.example` 加 `JWT_ISSUER`/`JWT_AUDIENCE`/`JWT_JWKS_URI` 具體值 + `VITE_KEYCLOAK_*` 模板;(6) runbook `docs/keycloak-setup.md`(5 step 手動指引,含 LDAP federation Adam-driven 步驟、frontend wiring deferred 章節、destructive cleanup constitutional consent 警告);(7) sub-plan `.claude/plans/v3-phase-1/m4-keycloak-sso-plan.md`(S1 done / S2 LDAP federation Adam-driven / S3 dashboard wiring pending / S4 destructive cleanup gated on S2+S3 proof);(8) tracker `m4-prod-ready-tracker.md` item #3 連到 sub-plan + runbook。**Backend 早 90% Keycloak-ready** — `services/authz-api/src/middleware/jwt.ts` 已含 JWKS RS256 decode + issuer alias support + `X-User-Id` fallback,只缺 env var 設定。Smoke 驗證:`/.well-known/openid-configuration` 200 回正確 issuer/jwks_uri;JWKS endpoint 200。**Constitutional checkpoint:** 沒碰 `authz_subject` 任何 row、沒改 LDIF、沒改 dev-seed.sql、沒移 `extractUser` X-User-Id fallback。S4 destructive cleanup 必須等 adam_ou e2e login 證實後另開 turn 取得 explicit consent。**LDAP federation 待 Adam 手動處理(原因:bind credentials 是 secret,sandbox 正確擋住 transcript 流入)** — runbook §3 列了 Keycloak Admin Console 設定步驟,Adam 自填 corporate LDAP creds(同 edgepolicy)。Plan: `.claude/plans/v3-phase-1/m4-keycloak-sso-plan.md`(S1 DONE / S2-S4 待後續 turn)。
- [x] **SEMANTIC-TYPE-DOWNGRADE-V01 + DAG-RUN-SMOKE-V01 (Hybrid type model + Tier 3 smoke + golden-case seed fix)** — 2026-04-29。Adam 在 Composer 試 material_search_fan-out Run all 看到三個錯誤 toast，逼出兩件事：(A) `semantic_type` 撞名（status 已撞 work-order vs shipment）+ 沒有 SSOT 的維運成本問題；(B) 我宣稱 V081 done 但**從沒 e2e 跑過 Run all**。**Q1（Hybrid type model 降級）:** `apps/.../utils/handleCompat.ts` 重構新增 `checkHandleCompat(out, inp): { level: 'ok'|'warn'|'block', reason?, outKind, inKind }`，**pgType family 變硬規（block）、semantic_type 強制相同變 advisory（warn allowed）**。`isCompatibleHandle` 保留 boolean 介面（`level !== 'block'`）給既有 callers 不破。`services/.../lib/dag-validate.ts` 把 `type_mismatch` 拆兩支 — pgType family mismatch severity='error'（擋 save）、純 semantic mismatch severity='warn'（advisory，不擋）。`DagTab.tsx` `onConnect` 走 tri-state：block→toast error+拒絕、warn→toast info「Advisory: …connection allowed」+ 加 amber dashed edge、ok→silent + 原 semantic 顏色 edge；FunctionNode 拖拽 input ring 三色（綠 ok / 琥珀 warn / 紅 dim block）+ Handle title=compat.reason；canvas 上方 Types legend 加 subtitle `advisory · pgType is the hard rule` + 整 row title hover 解釋。**為什麼這個方向長期成本最低：** PG 是 pgType 的 SSOT — 不撞名、零維護、runtime bind 自動對齊；semantic_type 留著做顏色 + `/compatible` palette suggest 但不再 block 連接，Curator 可選擇填 metadata 也可不填。撞名（status）變 cosmetic 不再是 logic bug。未來如果 domain 誤接真的咬到再升級到 `{domain, attr}` 結構，路徑開放。**Q2（Tier 3 smoke）:** 新檔 `services/authz-api/scripts/test-dag-run.ts` — GET dag → topo sort → 對每個 fn node POST `/api/dag/execute-node` accumulate upstream → 4xx 時 dump 完整 server `detail`。同時 `apps/authz-dashboard/src/api.ts:38` 改成 `body.detail ? \`${head}: ${detail}\` : head` — toast 不再吃掉 PG 真錯誤。**Tier 3 smoke 立刻打臉我先前的「fn 不存在」猜測，抓到 3 個真錯誤都是 seed bug：**(1) n1 `p_keywords` 是 `text[]` 不是 `text`，seed 送 string `'PC7250020H...'`，PG `malformed array literal`；(2) n2 真實 PG arg name 是 `p_searchkey` 不是 `p_material_no`，seed 跟 edge 都寫錯名 → `Unbound required input`；(3) n3 cascade fail 因為 n1 沒 row 給下游。**修法**：`database/seed/dag_material_search_fanout.sql` 改 (a) p_keywords pgType=`text[]` + bound_params 用 `jsonb_build_array(...)` 包成 array；(b) n2 input name + e1 targetHandle 都改成 `p_searchkey` + pgType=`character varying`；(c) n3 不動。重 apply seed 後 smoke 三 node 全綠：n1 (1 row, 4.9s)、n2 (4 rows, 68ms)、n3 (18 rows, 2.4s)。**長期教訓：** Tier B platform primitive 一律要求 Tier 3 minimum（API smoke 打真依賴），不是 TS clean 就算 done。我之前 V081「runtime smoke pending Adam restart」是 over-claim 的訊號 — 下次 AC 卡住直接降規（「我只到 Tier 2」）比假裝完成好。Plan: `.claude/plans/v3-phase-1/sink-as-authz-resource-plan.md` (AC-12 從 in_progress 真正 close、AC-8 補上 PROGRESS.md 同步)。
- [x] **SINK-AS-AUTHZ-RESOURCE-V081 (page sink 寫進 authz_resource，補 ModulesTab leaf 缺口)** — 2026-04-29。COMPOSER-SINK-V01 §D4 技術債一次性結清。問題：`authz_ui_page` 與 `authz_resource` 是兩棵獨立 FK 樹，sink 存的 page 只進 ui_page、ModulesTab 看不見、reload 後找不回。**完整方案**（不走 `_orphan_pages` workaround）：(1) **Migration `V081__sink_as_authz_resource.sql`** — backfill 把 dag-origin 的 `authz_ui_page` 鏡射成 `authz_resource(resource_type='page', resource_id='page:'+page_id)`，parent_id 取 dag.parent_id（而非 plan 原寫的 fn.parent_id；理由：page 與 dag 應為同 module 兄弟，避免 page 散到 schema 層級），fallback `module:pg_tiptop_v1`；同時新增 `modules_home:pages` UI descriptor。發現 `'page'` 已在 V067 cascade resolver enum 內，**不需放寬 CHECK constraint**，V070/V079 cascade 自動接上。Apply：2 backfill rows + descriptor inserted clean。(2) **Sink runtime dual-write** — `services/authz-api/src/lib/sink-runtime.ts` 加 `derivePageParentResource(client, dag_id)` helper，`emitPageSnapshot` 改走 `pool.connect()` BEGIN/COMMIT/ROLLBACK 單 tx，UPSERT 鏡射 `authz_resource` row + best-effort `refresh_module_tree_stats()` 在 commit 後 fire-and-forget；result 多 `authz_resource_id`/`authz_parent_id` 兩欄。(3) **API `GET /api/modules/:id/details`** — `services/authz-api/src/routes/modules.ts` 加 `childPages` query（讀 `parent_id=$1 AND resource_type='page' AND is_active`），response `children` 多 `pages: []`。(4) **Frontend** — `apps/authz-dashboard/src/api.ts` `ModuleDetails.children` 加 `pages` type；`components/modules/ModuleDetail.tsx` `sectionDataCount` 加 `pages: children.pages?.length`，新 'pages' sub-tab 走 `EmptyState`/table 二態，row click `dispatchEvent('open-auto-page', {page_id})` 復用既有 ConfigEngine 路徑（reload-safe via DB lookup）。**Plan §3.0 Decisions ratified during execution（3 處與 draft 偏離）：**(a) 用 `'page'` 而非新建 `'ui_page'` enum（已存在 + cascade 已 wired）；(b) FALLBACK_PAGE_PARENT 用 `module:pg_tiptop_v1` 而非新建 `_orphan_pages`（避免 dead end）；(c) parent_id 用 dag.parent_id 而非 fn.parent_id（語意更貼近使用者導航）。**已做：** AC-1/2/4 ✅、AC-8 ✅(本條)；**未做（已記錄 reason）：** AC-3 URL-level page persistence (defer to Tier 2 admin form wizard)、AC-5 V079 cascade integration test for `resource_type='page'`、AC-6 Playwright e2e、AC-7 docs sync (`docs/api-reference.md` + `docs/er-diagram.md`)。**待 Adam 驗證：** authz-api dev server (PID 61000) 跑 stale code，需重啟後 (a) curl `/api/modules/module%3Apg_tiptop_v1/details` 確認 `children.pages` ≥ 2；(b) Composer 開 `dag:material_search_fanout` → save-as-page → reload → ModulesTab `pg_tiptop_v1 → Pages` 子分頁可見。Plan: `.claude/plans/v3-phase-1/sink-as-authz-resource-plan.md` (IN-REVIEW)。
- [x] **FEEDBACK-V01 (Tier A primitive #3 — `authz_feedback` per-user × per-page in-app feedback)** — 2026-04-29。`two-tier-platform-model.md` §82 列的第三個 Tier A primitive,reuse SAVED-VIEW-V01 的 per-user × per-page row-level table pattern。問題:ConfigEngine 上線後 end-user 沒有 in-app 反饋管道,Curator 只能靠口頭/Slack 拿到 sample-bias 的抱怨。Schema `database/migrations/V082__authz_feedback.sql` 加 `authz_feedback (feedback_id PK, user_id, page_id, target_path, kind, body, status='open', curator_id, resolved_at, created_at, updated_at)` + **6 條 PG-side CHECK constraint**(user/page nonblank + `target_path ~ '^(page|column:.+|filter:.+)$'` + `kind IN (data_wrong/feature_request/confusing/other)` + `status IN (open/triaged/resolved/dismissed)` + `body length 1-4000`)+ 2 indexes(`(status, page_id)` for Curator inbox / `(user_id, page_id, created_at DESC)` for self-list)。**Append-only user model**:沒有 user PATCH/DELETE 自己 feedback;Curator 經 PATCH status 走 4-status flow(any-to-any、無 state machine、誤標可逆 + audit 序列完整)。Backend `services/authz-api/src/routes/feedback.ts`(NEW)4 routes:`POST /` (任 authenticated user 自寫) / `GET /mine?page_id=X` (self-scope `WHERE user_id=current`) / `GET /inbox?status=&page_id=` (`requireRole('ADMIN','AUTHZ_ADMIN')` 內部 middleware) / `PATCH /:id/status` (`requireRole`、`resolved_at = COALESCE(resolved_at, now())` 第一次 triage 才寫、後續保留歷史)。Mounted under `app.use('/api/feedback', requireAuth, feedbackRouter)` 對齊 saved-view pattern。Audit:`tier_a_feedback_create / triaged / resolved / dismissed` 4 條 action 寫 `authz_admin_audit_log`,actor_type='human'。Frontend `apps/authz-dashboard/src/api.ts` 加 4 method (`feedbackCreate/Mine/Inbox/PatchStatus`) + `FeedbackKind`/`FeedbackStatus`/`FeedbackRow` types;`hooks/useFeedback.ts`(NEW,minimal 32 行只 expose submit/submitting/error/lastSubmitted/reset);`components/FeedbackButton.tsx`(NEW,~150 行 floating bottom-right `MessageSquarePlus` 按鈕 + modal dialog,kind dropdown + 4000-char textarea + char counter + 1.2s success toast);wire 進 `ConfigEngine.tsx` `TablePageWithSavedView` 內(對齊 saved_view scope:ConfigEngine `columns_override` layout=table 的頁;descriptor / handler-driven 頁延後)。Smoke `services/authz-api/scripts/test-feedback.ts`(NEW)10/10 pass:create 201 + status=open + curator_id NULL / bad kind 400 / bad target_path 400 / empty body 400 / `/mine` self-scope (USER_A 不見 USER_B row) / `/inbox` non-admin 403 / `/inbox` admin sees ≥ 2 rows / PATCH triaged → curator_id=admin + resolved_at set / PATCH non-admin 403 / audit_log 抓到 create+triaged。tsc clean × 2。**Caveats**:(1) **AC-7 frontend round-trip 未在瀏覽器手動驗證** — component+hook+wire 已完成 + tsc clean,per `feedback_ui_verification` 不 block,commit body 顯式註記;(2) **Curator Inbox tab UI 切 follow-up commit `FEEDBACK-V01-INBOX-FU`**(避免 SAVED-VIEW timing 風險重演,Curator-side UX 設計面 — 排序、status setter、空狀態、pagination — 集中後做);(3) v1 frontend 只送 `target_path='page'`,column-level「📝」icon / filter-level icon 屬 v2(roadmap §3.2 deferred,等 page-level 真實使用率再評估);(4) `curator_id` 不設 FK,對齊 V080 `user_id` 慣例(subject 來源異質)。Plan: `.claude/plans/v3-phase-1/tier-a-feedback-plan.md` (DONE,單 session DRAFT→DONE)。
- [x] **SAVED-VIEW-V01 (Tier A primitive — `authz_user_view` per-user × per-page filters/sort/hidden_cols)** — 2026-04-29。`two-tier-platform-model.md` §82 列的第二個 Tier A primitive。Schema 走獨立 table(per-user × per-page state，**非** JSONB-only convention，因為 row-level 寫頻 + 跨頁列舉需求)：`database/migrations/V080__authz_user_view.sql` 加 `authz_user_view (view_id PK, user_id, page_id, name, config_json, is_default, created_at, updated_at)`，含 `(user_id, page_id, name) UNIQUE` 防同名 + **partial unique index `WHERE is_default = true`** 確保每 (user, page) 至多一個 default + 3 條 nonblank CHECK + `(user_id, page_id)` 一般 index。Backend `services/authz-api/src/routes/saved-view.ts`(NEW)6 routes：GET / (list by page_id) / GET /:view_id / POST / (create with optional `is_default:true` 觸發 transaction demote-then-insert) / PATCH /:view_id (rename + config_json) / POST /:view_id/set-default (transaction) / DELETE /:view_id；**self-scope authz**：所有 query `WHERE user_id = current_user`，cross-user 或 wrong page_id 一律 404 (no enumeration leak)。`config_json` shape：`{filters?: [{field,op,value}], sort?: {col,dir}, hidden_cols?: string[]}`，`isPlainConfig()` 驗 outer shape (inner field/op/value v1 op=eq only，深度驗證已記錄為 minor 技術債)。Audit：`tier_a_saved_view_create/update/set_default/delete` 4 條 action 寫 `authz_admin_audit_log`。Mounted under `requireAuth` at `app.use('/api/saved-view', ...)`。Frontend：`apps/authz-dashboard/src/api.ts` 加 6 method (`savedViewList/Get/Create/Update/SetDefault/Delete`) + `SavedViewConfig`/`SavedView` types；`hooks/useSavedView.ts`(NEW) 管 list/active/CRUD state，bootstrap 順序 `?view=<id>` URL > `is_default` > none，cross-user view_id 自動 fallback；`components/SavedViewBar.tsx`(NEW) toolbar 含 dropdown(★ default 標記)+ Update/Set-default/Rename/Delete/Save-as buttons + `Default` checkbox + lucide icons (Bookmark/Star/Pencil/Trash2/Save/Plus/X)；`components/ConfigEngine.tsx` `DataTable` 內部 `useState` (sortKey/sortDir/filterValues) 透過新 `initialSort/initialFilters/initialHiddenCols` props + `onStateChange` callback 上提，`hiddenCols` 走 `visibleColumns = useMemo(() => columns.filter(c => !hiddenCols.includes(c.key)))` 套用到 header/body/colSpan；新元件 `TablePageWithSavedView`(~85 行) 包 DataTable + SavedViewBar，URL `?view=<id>` 經 `URLSearchParams` 一次性解析 + `window.history.replaceState` 雙向同步(無 router 依賴)，view 切換經 `key={activeView?.view_id || 'no-view'}` 強制 DataTable remount。Smoke `services/authz-api/scripts/test-saved-view.ts`(NEW) 10/10 pass：create 201 / duplicate name 409 / list / set-default / partial-unique 強制單 default / cross-user 404 / PATCH rename / DELETE 200 / audit_log 寫入驗證 / bad config_json 400。tsc clean × 2。**Caveats**：(1) **AC-5 frontend round-trip 未在瀏覽器手動驗證** — component+hook 已完成 + tsc clean，但 round-trip 未 e2e（`feedback_ui_verification`：不 block，但 commit body 顯式註記）；(2) `is_shared` 從 v1 拿掉(don't design for hypothetical)；(3) 僅限 ConfigEngine `columns_override` 走 layout=table 的頁，descriptor / handler-driven 頁延後 per roadmap §3.1；(4) `isPlainConfig` 只驗外殼，inner shape 細化為技術債(v1 op=eq only)。Plan: `.claude/plans/v3-phase-1/tier-a-saved-view-plan.md` (READY-FOR-REVIEW)。
