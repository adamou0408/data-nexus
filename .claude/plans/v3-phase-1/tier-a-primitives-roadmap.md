...
| **A3** | feedback | §82 #3 | end-user → Curator | ✅ DONE (FEEDBACK-V01, 2026-04-29) | V082 + 4 routes + smoke 10/10 + FeedbackButton wire 進 `TablePageWithSavedView`;Curator Inbox tab 切 FU commit `FEEDBACK-V01-INBOX-FU` |
...
| ✅ | ~~**A3 — feedback**~~ | DONE 2026-04-29 (FEEDBACK-V01) | — | 「這欄錯了」沒地方提(★★★) |
...
| 2026-04-29 | Adam (executor) | A3 row → DONE | FEEDBACK-V01 落地:V082 + `/api/feedback` 4 routes + smoke 10/10 + tsc×2 clean + `FeedbackButton` 浮動 UI + `useFeedback` hook + wire 進 `TablePageWithSavedView`;append-only user 模型、any-to-any status flow、PG-side CHECK constraints。**AC-7 frontend round-trip 未在瀏覽器手動驗證** caveat;Curator Inbox tab UI 切 FU commit `FEEDBACK-V01-INBOX-FU` |
