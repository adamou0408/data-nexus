...
- AI auto-deploy / blessing workflow — Constitution §9.3 明文禁止；要走 Article 8 修訂
...
- [ ] **AC-12:** Constitution §9.2 / §9.3 / §9.6 / §9.9（讀取/寫入/audit/eval-capture）對齊 — implementation comment 引用對應條文
...
|------|-----------|------------|
| AI 產 SQL 含 `DROP` / `TRUNCATE` 試圖偷渡 destructive 操作 | 中 | `ai-call.ts` 在 return 前 regex 黑名單拒絕（DROP/TRUNCATE/GRANT/REVOKE/COPY），命中則 raise；Constitution §9.3 寫入只准 CREATE OR REPLACE FUNCTION |
...
| Adam 把 generated SQL 直接 deploy 而沒檢查 → bad function 進 prod | 中 | 既有 Deploy 已要 window.confirm；本 plan 不改變那條 path；Constitution §9.3 sandbox→review 對應現實上是 Adam 看 SQL+按 Deploy |
