...
- [x] **Render-token registry (V053 + RenderTokensContext, 2026-04-26)** — ICON_MAP / STATUS_COLORS / PHASE_COLORS / GATE_COLORS 從 hardcoded 搬進 `authz_ui_render_token`,Curator 可 INSERT 新 token 零 React 改動
...
- [x] **DAG → Tier B page bridge Path A (DAG-SAVE-PAGE-01, 2026-04-26)** — DagTab Inspector「Save as page」按鈕,把任一 node 的 `last_result` 凍進 `authz_ui_page.snapshot_data` (V054)。零 React 改動 = Curator workflow loop 已通(run DAG → 存頁 → 開頁)。Path B/C/D(live re-execution / saved_view 整合 / semantic_type drilldown)排在 saved_view primitive 後
...
| ~~Curator 想加 icon 要動 .tsx~~ | ~~平台脆化~~ | ✅ V053 + RenderTokensContext (2026-04-26) — Curator 改 SQL 即可 |
