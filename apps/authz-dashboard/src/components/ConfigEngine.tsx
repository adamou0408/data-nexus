...
//
// NOTE (catalog Phase 2, 2026-05-04): The legacy in-file `PublishedDagPage`
// renderer has been removed. Live published_dag pages are rendered by
// `catalog/DetailView.PublishedDagBody` via <CatalogWorkspace preset="pages">.
// This component no longer handles the `published_dag_id` branch.
// ============================================================
  ...
  return (
    <div>
      ...
      {hasHandler ? (() => {
        ...
        // Defensive guard: published_dag pages must be opened via
        // <CatalogWorkspace preset="pages"> — see catalog/DetailView.PublishedDagBody.
        // If we land here it means a drilldown / route still points at ConfigEngine
        // for a published_dag page; surface it instead of silently rendering nothing.
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700 flex items-start gap-2">
          <AlertTriangle size={16} className="shrink-0 mt-0.5" />
          <div>
            Page <code className="font-mono">{current.pageId}</code> is a published DAG.
            Open it from the Pages tab (Catalog) instead.
          </div>
        </div>
      ) : (
        <>
